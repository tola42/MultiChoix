...
;~ #AutoIt3Wrapper_Res_Icon_Add=shell32_331-16x16.ico,1001,myIkone
;~ #AutoIt3Wrapper_Res_File_Add=myIkone-16x16px.jpg,rt_rcdata,myIkone2
;~ #AutoIt3Wrapper_Res_File_Add=myIkone-16x16px.jpg, rt_rcdata, TEST_JPG_3
;~ #AutoIt3Wrapper_Res_File_Add=myIkone-22x22px.jpg, rt_rcdata, LOGO_JPG
#AutoIt3Wrapper_Res_File_Add=Params-32x32.png, rt_rcdata, LOGO_PNG
#AutoIt3Wrapper_Run_After=copy "%out%" "E:\LClogs"
#AutoIt3Wrapper_Run_After=copy "%outx64%" "E:\LClogs"
#AutoIt3Wrapper_Run_After="E:\LCLogs\%scriptfile%.exe"
...
If IsArray($arrActions) Then
	...
	ConsoleWrite("Les clés : " & _ArrayToString($arrActionsKey) & @CRLF & @CRLF)
	ConsoleWrite("Les valeurs : " & _ArrayToString($arrActionsValue) & @CRLF & @CRLF)
;~ 	_test()
...
;~ 	Local $hMainGUI = GUICreate("---", 290, 24, -1, -1, BitOR($DS_MODALFRAME, $WS_POPUPWINDOW), BitOR($WS_EX_TOPMOST, 0))
...
;~ 	$iParamsButton = GUICtrlCreateButton("Pars", 238, 1, 32, 22, BitOR($BS_ICON, $BS_FLAT,$SS_NOTIFY))
	$iParamsButton = GUICtrlCreatePic("", 238, -3, 32, 32)
	_ResourceSetImageToCtrl(-1, "LOGO_PNG")
...
Func OKButton()
	...
	MsgBox($MB_OK, "GUI Event", _ArraySearch ($arrActions,GUICtrlRead( $idComboBox)))
EndFunc   ;==>OKButton

Func _TestButtonPic()
	Local $aInfo = GUIGetCursorInfo()
	If $aInfo[4] = $iParamsButton Then _Params()
